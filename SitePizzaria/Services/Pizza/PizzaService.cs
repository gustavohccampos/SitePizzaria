...
namespace SitePizzaria.Services.Pizza
{
    public class PizzaService : IPizzaInterface
    {
        ...
        public string GeraCaminhoArquivo(IFormFile foto)
        {
            ...
            var nomeCaminhoImagem = codigoUnico + ".png";
            //foto.FileName.Replace(" ", "").ToLower()+ 
        ...
        public async Task<List<PizzaModel>> GetPizzasFiltro(string? pesquisar)
        {
            try
            {
                ...
                var pizzas = await _context.Pizzas.Where(pizzaBanco => pizzaBanco.Nome.Contains(pesquisar)).ToListAsync();
                return pizzas;
